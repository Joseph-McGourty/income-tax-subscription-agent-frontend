...
class ClientAlreadySubscribedController @Inject()(val baseConfig: BaseControllerConfig,
  ...
  val show: Action[AnyContent] = Authorised.asyncCore { implicit user =>
    implicit request =>
      Future.successful(Ok(views.html.client_already_subscribed(postAction = controllers.routes.ClientAlreadySubscribedController.submit())))
  }
